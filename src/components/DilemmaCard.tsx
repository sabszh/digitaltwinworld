      ...
      <div className="mt-2 flex flex-wrap items-center gap-3">
        ...
        {value.trim() && (
          <button
            ...
            className="inline-flex rounded-full bg-[var(--accent)] px-4 py-2 text-sm font-semibold text-slate-950 shadow-lg shadow-sky-950/20"
          >
    ...
    <section className="relative z-20 flex h-dvh items-end justify-center px-4 py-4 pt-20 md:items-center md:justify-end md:px-8 md:py-5">
      <div className="surface-panel flex max-h-full w-full max-w-[min(560px,calc(100vw-2rem))] flex-col overflow-hidden rounded-3xl p-5 md:p-5">
        <div className="min-h-0 overflow-y-auto overscroll-contain pr-2 [scrollbar-gutter:stable]">
          ...
          <h2 className="font-editorial mt-4 text-[26px] font-semibold italic leading-tight text-[var(--text)] md:text-[32px]">{dilemma.title}</h2>

          <p className="mt-3 text-[15px] font-normal leading-6 text-[var(--muted)]">{dilemma.scenePrompt}</p>

          <div className="mt-5 pb-1">
            {framed.lead && <p className="text-[13px] leading-5 text-[var(--faint)]">{framed.lead}</p>}
            <p className="mt-1 text-[18px] font-semibold leading-6 text-[var(--text)]">{framed.question}</p>
